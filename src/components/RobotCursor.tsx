...
export default function RobotCursor() {
  ...
  // Unified position tracking, device detection, and hover interactions
  useEffect(() => {
    let hasTouch = false;

    const updatePosition = (clientX: number, clientY: number, isTouch: boolean) => {
      setIsVisible(true);
      if (isTouch) {
        // Offset upwards on touchscreens so user's finger doesn't block the visual screen
        mouseX.set(clientX);
        mouseY.set(clientY - 75);
      } else {
        // Offset to the side for desktop cursors
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      if (e.touches && e.touches.length > 0) {
        ...
        // Also check if they tapped an interactive element
        checkInteractiveTarget(e.target as HTMLElement);
      }
    };

    const handleTouchEnd = () => {
      // Clear hover when lifting finger
      setIsHovered(false);
      setHoverText('');
    };
    ...
    // Register Desktop Event Listeners
    window.addEventListener('mousemove', handleMouseMove, { passive: true });
    document.addEventListener('mouseover', handleMouseOver);
    document.addEventListener('mouseout', handleMouseOut);

    // Register Mobile Touch Event Listeners
    window.addEventListener('touchstart', handleTouchStart, { passive: true });
    window.addEventListener('touchmove', handleTouchMove, { passive: true });
    window.addEventListener('touchend', handleTouchEnd, { passive: true });
    window.addEventListener('touchcancel', handleTouchEnd, { passive: true });

    // Initial cursor styling for desktop
    const isTouchDeviceInitial = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
    if (!isTouchDeviceInitial) {
      setIsVisible(true);
      document.documentElement.classList.add('custom-cursor-active');
    } else {
      // Start as invisible on touch device, but make it visible as soon as they touch the screen
      setIsVisible(false);
    }
